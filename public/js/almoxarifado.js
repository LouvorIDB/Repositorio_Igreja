    ...
    if (!almoxarifadoDocs || almoxarifadoDocs.length === 0) {
        container.innerHTML = `
            <div class="col-span-full py-12 text-center bg-slate-900/40 border border-slate-800 rounded-2xl">
                ...
                <p class="text-slate-500 text-xs mt-1">Líderes podem cadastrar instruções de como ligar a mesa, senhas do roteador e PDFs.</p>
            </div>
        `;
        return;
    }
            ...
            <div class="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col justify-between space-y-3">
                <div>
                    <div class="flex items-center justify-between gap-2 mb-2">
                        <span class="text-xs text-slate-400 font-medium">📁 ${minNome}</span>
                        ${doc.content_url ? `<a href="${doc.content_url}" target="_blank" class="text-xs text-brand-400 hover:underline flex items-center gap-1 font-medium">Abrir Link ↗</a>` : ''}
                    </div>
